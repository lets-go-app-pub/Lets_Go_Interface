...
void TimeOutUser::timeOutUser(
    ...
    function_thread = std::make_unique<std::jthread>([
        ...
        request.set_inactive_message(inactive_message);
        ...
        auto status = request_fields_stub->TimeOutUserRPC(&context, request, &response);
        ...
        if (stop_token.stop_requested()) {
            ...
            const std::string errorMessage = "Time out user failed..\nGrpc returned status.ok() == false; code: " +
            ...
            std::string errorMessage = "Time out user failed.\n" + response.error_message();
        ...
        emit signal_internalCompleted();
        emit signal_reportRequestSuccessfullyCompleted(
                user_oid,
                inactive_message,
                response
        );
