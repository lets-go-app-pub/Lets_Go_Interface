...
void SetServerCategory::runSetServerCategory(
    ...
    function_thread = std::make_unique<std::jthread>([delete_this, category_name, icon_display_name, order_number, min_age, color, completed_lambda, this] (
        ...
        request.set_display_name(category_name);
        request.set_icon_display_name(icon_display_name.empty() ? category_name : icon_display_name);
        request.set_order_number(order_number);
        ...
        auto writer = request_fields_stub->SetServerCategoryRPC(&context, &response);
        ...
        if(stop_token.stop_requested()) {
            ...
            const std::string errorMessage = "Set server Category failed..\nGrpc returned status.ok() == false; code: " +
            ...
            std::string errorMessage = "Set server Category failed.\n";
